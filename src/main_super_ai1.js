...
function AI_tickFormationIntel(deltaMs, formationEnemies, frontline, aliveRatio, melee) {
  ...
  if (formationIntel.modeHoldMs <= 0) {
    ...
    if (EXP_TACTICS_V2 && leaderIsFlag && (formationIntel.tacticHeatMs || 0) <= 0 && invulnerabilityTimer <= 0) {
      ...
      const allow = Phaser.Math.Clamp((0.25 + 0.55 * skill) * (mood === "punish" ? 1.0 : 0.55) * (1 - 0.85 * AI_clamp01(1 - (aiPressureMul || 1))) * (1 - 0.75 * AI_clamp01(aiPressureEwma || 0)), 0, 0.85);
...
const EXP_TACTICS_V2 = true;         // extra formation tactics (pinch/cross) — smart but still braked
