...
function AI_tickFormationIntel(deltaMs, formationEnemies, frontline, aliveRatio, melee) {
  ...
  if (formationIntel.flankHoldMs <= 0) {
    const px = (player && player.active) ? player.x : 400;
    const prefer = (px < cx) ? -1 : 1;

    const lt = formationIntel.leaderType || "blue";
    const leaderIsFlag = (lt === 'flagship');
    const feintP = leaderIsFlag ? 0.22 : 0.35; // non-flagships feint more (less disciplined)
  ...
  if (formationIntel.modeHoldMs <= 0) {
    ...
    const focusP = Phaser.Math.Clamp(0.55 + 0.25 * waveN + 0.20 * thin + (melee ? 0.10 : 0) + (leaderIsFlag ? 0.08 : 0), 0.45, 0.93);
    formationIntel.mode = POHP_grChance(focusP) ? "focus" : "sweep";
...
const POHP_GAME_VERSION = "GKD|RULESET=v1.3.1|BUILD=main.js";       // bump when you change gameplay-affecting logic       // bump when you change gameplay-affecting logic       // bump when you change gameplay-affecting logic
...
window.__CHAIN_STATUS_CB = (msg) => {
  try {
    ...
    if (gamePhase === GAME_PHASE.GAME_OVER) {
      chainStatusText.setText(msg + "\nR = RESTART   B = SUBMIT PROOF (CHAIN)   P = DOWNLOAD PROOF");
    } else {
      chainStatusText.setText(msg);
    }
  } catch (e) {}
...
const ENEMY_MAX_DIVE_MS = 20000;// allow slow dives (ENEMY_SPEED_FACTOR) to reach bottom
...
waveBulletCap = Phaser.Math.Clamp(waveBulletCap, 3, capBase);
  ...
  function updateFormationShootTarget() {
    ...
    if (EXP_LANE_HEATMAP && formationIntel && Number.isFinite(formationIntel.focusLaneX)) {
      ...
      const mix2 = Phaser.Math.Clamp(moodMix * (0.65 + 0.55 * (aiLearn.level || 0)), 0.06, 0.34);
  ...
  const buildFormationVolleyPlan = (targetX) => {
    ...
    // “Real Galaxian vibe”: sometimes sweep from a flank, sometimes focus near target.
    const focus = (formationIntel && formationIntel.mode)
      ? (formationIntel.mode === 'focus')
      : (melee || POHP_grChance(0.58));
    const plan = [];
    if (focus) {
      plan.push(targetCol);
  ...
  if (formationVolleyLeft > 0 && formationVolleyGapMs <= 0) {
    ...
    const gapBase = Math.max(70, 140 - Math.floor((w - 1) * 6) - Math.floor((1 - aliveRatio) * 20) - (melee ? 20 : 0));
...
function maybeStartNextWave(scene) {
  ...
  // wave clear bonus (same logic as bullet-kill path)
...
function gameOver() {
  ...
  if (window.CHAIN?.enabled && window.ChainClient?.verifyRun) {
    window.ChainClient.verifyRun(POHP_lastPackage).catch(() => {
